...
class Renderer {
    ...
    Vector3 screen(Vector3 normalPos) { // -1..1 to
        return new Vector3(
    ...
    Vector3 project(Vector3 v, float zoom) {
        ...
        float z = v.z + zoom;
        float x = (v.x * fovFactor) / (z * aspect);
        float y = (v.y * fovFactor) / z;
        
        // fish eye distortion
        float dist = (float) Math.sqrt(x*x + y*y);
        float strength = 0f;  
        float distort = 1 + dist * strength;
        
        return new Vector3(x * distort, y * distort, z);
    }
}